...
class DataManager:
    ...
    def get_median_value(self, values: list) -> float:
        ...
        if n_values % 2 == 0:
            median = (
                sorted_values[n_values // 2 - 1] + sorted_values[n_values // 2]
            ) / 2
        else:
    ...
    def get_add_action_query_set(
        ...
        add_action_queryset = AddAction.objects.filter(
    ...
    def get_extract_action_query_set(
        ...
        extract_action_queryset = ExtractAction.objects.filter(
            criterion1 & criterion2 & criterion3
        )
        return extract_action_queryset

    def get_rounded_add_action_volumes(
    ...
    def get_rounded_reaction_volumes(
    ...
    def get_grouped_temperature_reactions(
        self, reactionqueryset: QuerySet[Reaction]
    ) -> dict:
        """
    ...
    def get_grouped_reaction_by_class_recipe(
        ...
        """
        unique_classes = self.get_unique_reaction_classes(
            reactionqueryset=reactionqueryset
        )
        unique_recipes = self.get_unique_reaction_recipes(
            reactionqueryset=reactionqueryset
        )
    ...
    def create_compound_order_model(
        self, order_df: pd.DataFrame, is_custom_starter_plate: bool = False
    ):
        """
