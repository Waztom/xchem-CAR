...
class PipetteManager:
    ...
    def get_pipette_type(
        self, rounded_volumes: list, channel_type: str = "single"
    ) -> dict:
        """
        ...
        try:
            ...
            logger.debug(
                f"Median volume for pipette selection: {median_transfer_volume}µL"
            )

            # Filter for pipettes that match the channel type
            compatible_pipettes = [
                p for p in self.pipettes_available if p["type"] == channel_type
            ]
            if not compatible_pipettes:
                logger.warning(
                    f"No pipettes found for channel type: {channel_type}, using defaults"
                )
                return self.pipettes_available[0]
            ...
            suitable_pipettes = [
                p
                for p in compatible_pipettes
                if p["maxvolume"] >= median_transfer_volume
            ]
            ...
            minimum_transfers = min(
                [
                    pipette_data["notransfers"]
                    for pipette_data in pipette_compare_dict.values()
                ]
            )

            optimal_pipettes = [
                pipette_name
                for pipette_name, pipette_data in pipette_compare_dict.items()
            ...
            if len(optimal_pipettes) > 1:
                ...
                optimal_pipettes = [
                    pipette_name
                    for pipette_name in optimal_pipettes
                    if pipette_compare_dict[pipette_name]["volumedifference"]
            ...
            selected_pipette = next(
                (
                    pipette
                    for pipette in self.pipettes_available
                    if pipette["labware"] == optimal_pipettes[0]
                ),
                suitable_pipettes[0],  # Fallback
            )

            logger.info(
                f"Selected pipette: {selected_pipette['labware']} for {channel_type} channel operations"
            )
        ...
        try:
            ...
            for volume in rounded_volumes:
                if volume <= pipette_volume:
                    ...
                    transfers_needed = (
                        volume + pipette_volume - 1
                    ) // pipette_volume  # Ceiling division
    ...
    def create_pipette_model(self):
        ...
        try:
            ...
            logger.info(
                f"Created pipette model: {pipette_obj.name} (ID: {pipette_obj.id})"
            )
            return pipette_obj
    ...
    def create_tiprack_model(self, name: str):
        ...
        try:
            ...
            logger.info(
                f"Created tiprack model: {tiprack_obj.name} at position {index_slot}"
            )
            return tiprack_obj
    ...
    def setup_pipettes_for_session(self, volumes: list, channel_type: str = "single"):
        ...
        try:
            ...
            logger.info(
                f"Successfully set up pipettes for session with {len(tipracks)} tipracks"
            )
            return True
