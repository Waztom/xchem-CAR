...
class BaseSession(ABC):
    ...
    def __init__(
        ...
        self.actionsession_ids = list(
            actionsessionqueryset.values_list("id", flat=True)
        )
        self.reaction_ids = list(
            actionsessionqueryset.values_list("reaction_id", flat=True).distinct()
        )
        self.actionsessiontype = actionsessionqueryset.values_list(
            "type", flat=True
        ).distinct()[0]
    ...
    def create_ot_session_model(self):
        ...
        try:
            ...
            logger.info(
                f"Created OT session: {otsessionobj.id} for step {self.reactionstep}"
            )
            return otsessionobj
