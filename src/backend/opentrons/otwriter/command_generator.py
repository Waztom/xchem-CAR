...
class CommandGenerator:
    ...
    def indent(self, level: int = 1) -> str:
        """Return proper indentation string."""
        return "    " * level
    ...
    def get_script_setup(self, protocolname: str, apiLevel: str) -> List[str]:
        ...
        """
        logger.info(f"Generating script setup for protocol '{protocolname}' with API level {apiLevel}")
    ...
    def get_labware_setup(self, plates: List[Dict]) -> List[str]:
        ...
        for plate in plates:
            ...
            logger.info(f"Adding plate '{platename}' ({labware}) at position {plateindex}")
            commands.append(
    ...
    def get_pipette_setup(self, pipette_name: str, pipette_labware: str, 
                        pipette_position: str, tiprack_names: List[str]) -> List[str]:
        ...
        """
        logger.info(f"Generating pipette setup for '{pipette_name}' ({pipette_labware}) at {pipette_position} position")
        logger.info(f"Pipette will use {len(tiprack_names)} tiprack(s): {', '.join(tiprack_names)}")
    ...
    def get_number_tips_available_setup(self, num_tipracks: int, channel_type: str) -> List[str]:
        ...
        logger.info(f"Setting up tip tracking for {channel_type} channel pipette with {num_tipracks} tiprack(s)")
    ...
    def get_pickup_tip_function(self, pipette_name: str) -> List[str]:
        ...
        return [
            ...
            f"\t\t\t{pipette_name}.reset_tipracks()",
            "\t\t\ttipstate[\"notipsavailable\"] = tipstate[\"maxnumbertips\"]",
            f"\t\tif not {pipette_name}.has_tip:",
            f"\t\t\t{pipette_name}.pick_up_tip()",
    ...
    def get_drop_tip_function(self, pipette_name: str) -> List[str]:
        ...
        return [
            "\n\n\tdef dropTip():",
            f"\t\tif {pipette_name}.has_tip:",
            f"\t\t\t{pipette_name}.drop_tip()"
        ]
    ...
    def delay_protocol(self, delay: int) -> List[str]:
        ...
        return [
            f"\n\t# {humanread}",
            f"protocol.delay(seconds={delay})"
        ]
    ...
    def set_aspirate_speed(self, speed: int) -> List[str]:
        ...
        ----------
        speed : int
            Aspirate speed in µL/s
            
        Returns
        -------
        List[str]
            Set aspirate speed commands
        """
        ...
        return [
            f"\n\t# {humanread}",
            f"{pipette_name}.flow_rate.aspirate={speed}"
        ]
    ...
    def set_dispense_speed(self, speed: int) -> List[str]:
        ...
        ----------
        speed : int
            Dispense speed in µL/s
            
        Returns
        -------
        List[str]
            Set dispense speed commands
        """
        ...
        return [
            f"\n\t# {humanread}",
            f"{pipette_name}.flow_rate.dispense={speed}"
        ]
    ...
    def transfer_fluid_single(
        ...
        logger.info(f"Transfer: {transvolume:.1f} µL from {aspirateplatename}[{aspiratewellindex}] to {dispenseplatename}[{dispensewellindex}]")
        ...
        if transvolume <= 0:
            logger.warning(f"Transfer volume is {transvolume} µL which is <= 0, this may cause errors")
            
        if transvolume > 1000:
            logger.warning(f"Transfer volume is {transvolume} µL which is unusually large")
        
        return [
            f"\n\t# {humanread}",
            f"{pipette_name}.transfer({transvolume}, {aspirateplatename}.wells()[{aspiratewellindex}].bottom({aspirateheight}), {dispenseplatename}.wells()[{dispensewellindex}].top({dispenseplatename}.highest_z*0.05), air_gap = {air_gap_volume}, touch_tip=True, new_tip='never', blow_out=True, blowout_location='destination well')"
        ]
    ...
    def transfer_fluid_multi(
        ...
        logger.info(f"Transfer: {transvolume:.1f} µL from {aspirateplatename} column {aspiratecolumnindex} to {dispenseplatename} column {dispensecolumnindex}")
        logger.info(f"Aspirate height: {aspirateheight} mm")
        ...
        return [
            f"\n\t# {humanread}",
            f"{pipette_name}.transfer({transvolume}, {aspirateplatename}.columns()[{aspiratecolumnindex}][0].bottom({aspirateheight}), {dispenseplatename}.columns()[{dispensecolumnindex}][0].top({dispenseplatename}.highest_z*0.05), air_gap = {air_gap_volume}, touch_tip=True, new_tip='never', blow_out=True, blowout_location='destination well')"
        ]
    ...
    def mix_well(self, wellindex: int, nomixes: int, plate: str) -> List[str]:
        ...
        if nomixes <= 0:
            logger.warning(f"Number of mixes is {nomixes}, which is <= 0. This may cause errors.")
        ...
        return [
            f"\n\t# {humanread}",
            f"{pipette_name}.mix({nomixes}, {mix_volume}, {plate}.wells()[{wellindex}])"
        ]
    ...
    def mix_column(self, columnindex: int, nomixes: int, plate: str) -> List[str]:
        ...
        if columnindex < 0 or columnindex > 11:  # Standard 96-well plates have 12 columns, indexed 0-11
            logger.warning(f"Column index {columnindex} may be out of range for standard plates")
        
        return [
            f"\n\t# {humanread}",
            f"{pipette_name}.mix({nomixes}, {mix_volume}, {plate}.columns()[{columnindex}][0])"
        ]