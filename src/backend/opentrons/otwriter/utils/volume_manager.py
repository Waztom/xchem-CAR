...
class VolumeManager:
    ...
    def get_well_volume_available(self, wellobj: Well) -> float:
        ...
        logger.info(f"Checking available volume in well {wellobj.index} of plate {plate_name} (ID: {plate_id})")
        ...
        self.update_well_available(
            wellvolumeavailable=well_volume_available, 
            wellobj=wellobj
        )
    ...
    def update_well_available(self, wellvolumeavailable: float, wellobj: Well):
        ...
        if wellvolumeavailable < 0:
            logger.warning(f"Well {wellobj.index} in plate {wellobj.plate_id.name} has insufficient volume")
    ...
    def update_well_volume(self, wellobj: Well, transfervolume: float):
        ...
        logger.info(f"Updating well {wellobj.index} volume: {original_volume} µL → {new_volume} µL (transfer: {transfervolume} µL)")
    ...
    def get_max_well_volume(self, plateid: int) -> float:
        ...
        logger.info(f"Maximum well volume for plate {plateobj.name}: {max_well_volume} µL")
    ...
    def get_dead_volume(self, maxwellvolume: float) -> float:
        ...
        logger.info(f"Dead volume calculated as {dead_volume} µL (5% of {maxwellvolume} µL)")
        return dead_volume
    
    def calculate_aspirate_height(self, bottomlayervolume: float, labware: str) -> float:
        """
        ...
        """
        logger.info(f"Calculating aspirate height for {labware} with bottom layer volume {bottomlayervolume} µL")
        
        try:
            aspirateheightconvesion_m = labware_plates[labware]["aspirateheightconversion-m"]
            aspirateheightconvesion_c = labware_plates[labware]["aspirateheightconversion-c"]
            ...
            logger.error(f"Labware type '{labware}' not found in labware_plates dictionary")
    ...
    def get_optimal_transfer_volume(self, available_volume: float, requested_volume: float) -> float:
        """
        ...
        """
        logger.info(f"Determining optimal transfer volume: requested={requested_volume} µL, available={available_volume} µL")
        
        if available_volume >= requested_volume:
            logger.info(f"Sufficient volume available, transferring requested {requested_volume} µL")
            return requested_volume
        
        # If not enough volume, return the available amount
        optimal_volume = max(0, available_volume)
        logger.warning(f"Insufficient volume! Requested: {requested_volume} µL, available: {available_volume} µL")
        logger.info(f"Will transfer {optimal_volume} µL instead of requested {requested_volume} µL")
    ...
    def update_well_reactant_status(self, wellobj: Well, is_for_next_step: bool):
        ...
        logger.info(f"Updating well {wellobj.index} in plate {wellobj.plate_id.name}: marking as {status_text} for next step")
    ...
    def update_column_reactant_status(self, columnobj, is_for_next_step: bool):
        ...
        status_text = "available" if is_for_next_step else "not available"
        logger.info(f"Updating all wells in column {columnobj.index}: marking as {status_text} for next step")
        
        wellqueryset = Well.objects.filter(column_id=columnobj)
        well_count = wellqueryset.count()
        
        if well_count:
            logger.info(f"Found {well_count} wells to update in column {columnobj.index}")
