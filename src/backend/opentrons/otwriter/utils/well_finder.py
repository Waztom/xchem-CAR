...
class WellFinder:
    ...
    def find_reaction_well(self, reaction_id: int, well_type: str) -> Well:
        ...
        """
        logger.info(f"Finding reaction well for reaction ID {reaction_id}, type {well_type}")
        ...
        try:
            ...
            logger.info(f"Found well (ID: {well_obj.id}) for reaction {reaction_id} in plate {well_obj.plate_id.name}, well index {well_obj.index}")
            return well_obj
        except Well.DoesNotExist:
            logger.error(f"Could not find well for reaction {reaction_id}, type {well_type}")
            logger.error(f"Searched with params: otsession_id={self.script_generator.otsession_id}, reaction_id={reaction_id}, type={well_type}")
            raise
    ...
    def find_solvent_wells(self, solvent: str, transfer_volume: float) -> List[List]:
        ...
        """
        logger.info(f"Finding solvent wells for {solvent}, volume needed: {transfer_volume} µL")
        well_info = []
        ...
        try:
            ...
            if not solvent_plate_queryset.exists():
                logger.warning(f"No solvent plates found for session ID {self.script_generator.otsession_id}")
                return well_info
            ...
            for solvent_plate in solvent_plate_queryset:
                ...
                if not well_queryset.exists():
                    logger.warning(f"No wells with {solvent} found in plate {solvent_plate.name}")
                    continue
                    
                logger.info(f"Found {well_queryset.count()} wells with {solvent} in plate {solvent_plate.name}")
            ...
            for well_obj in well_objs:
                ...
                if well_volume_available > 0:
                    if well_volume_available >= remaining_volume:
                        logger.info(f"Taking {remaining_volume} µL from well {well_obj.index} in plate {well_obj.plate_id.name}")
                        volume_manager.update_well_volume(
                            wellobj=well_obj, 
                            transfervolume=remaining_volume
                        )
                        well_info.append([well_obj, remaining_volume])
                        remaining_volume = 0.00
                        
                    elif well_volume_available < remaining_volume:
                        logger.info(f"Taking partial volume {well_volume_available} µL from well {well_obj.index} in plate {well_obj.plate_id.name}")
                        volume_manager.update_well_volume(
                            wellobj=well_obj, 
                            transfervolume=well_volume_available
                        )
    ...
    def find_starting_material_wells(
        ...
        """
        logger.info(f"Finding starting material wells for reaction {reaction_id}, step {reaction_step_no}")
        ...
        if previous_reaction_queryset:
            logger.info(f"Found {len(previous_reaction_queryset)} previous reaction(s) for this material")
        ...
        try:
            ...
            if not well_objects.exists():
                logger.warning("No starting material wells found with initial search criteria")
            else:
                logger.info(f"Found {well_objects.count()} starting material well candidates")
            ...
            if reaction_step_no > 1 and not well_info:
                logger.info("This is a later reaction step - searching for wells in reaction/workup plates")
                try:
                    ...
                    if well_queryset.exists():
                        ...
                        logger.info(f"Found material in {well_obj.type} plate, well {well_obj.index}")
                        well_info.append([previous_reaction_queryset, well_obj, transfer_volume])
                    else:
        ...
        if not well_info:
            logger.warning(f"No starting plate well info found for SMILES: {smiles[:20]}...")
            
            starter_well_smiles_available = Well.objects.values_list("smiles", flat=True).filter(
                otsession_id=self.script_generator.otsession_id,
                type="startingmaterial",
                available=True,
            ).distinct()
            
            logger.warning(f"Available starting materials: {len(starter_well_smiles_available)} different compounds")
        else:
            logger.info(f"Found {len(well_info)} well(s) for starting material transfer")
    ...
    def _process_well_objects(
        ...
        for well_obj in well_objects:
            ...
            wells_processed += 1
            well_volume_available = volume_manager.get_well_volume_available(wellobj=well_obj)
            ...
            if well_volume_available > 0:
                if well_volume_available >= remaining_volume:
                    ...
                    well_info.append([previous_reaction_queryset, well_obj, remaining_volume])
                    remaining_volume = 0.00
                    ...
                    well_info.append([previous_reaction_queryset, well_obj, well_volume_available])
                    remaining_volume = remaining_volume - well_volume_available
                    logger.info(f"Still need {remaining_volume} µL")
            else:
                logger.info(f"Well {well_obj.index} has insufficient volume available")
