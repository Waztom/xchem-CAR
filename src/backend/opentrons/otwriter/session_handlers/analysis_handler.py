...
class AnalysisSessionHandler(SessionHandler):
    ...
    def process_session(self, actionsession_queryset: QuerySet) -> None:
        ...
        logger.info(f"Processing analysis step {reaction_step}, session {session_number}")
        ...
        for i, actionsession_obj in enumerate(actionsession_queryset):
            ...
            logger.info(f"Processing analysis for reaction {reaction_id} ({i+1}/{action_count})")
            reaction_obj = getReaction(reaction_id=reaction_id)
            self.process_analysis_actions(actionsession_obj, reaction_obj, session_number)
    ...
    def process_analysis_actions(self, actionsession_obj, reaction_obj, session_number):
        ...
        logger.info(f"Processing analysis for reaction {reaction_id} (class: {reaction_class}, recipe: {recipe_type})")
        ...
        try:
            ...
            try:
                analysis_actions = [
                    ...
                    if actionsession["type"] == "analyse" and actionsession["sessionnumber"] == session_number
                ][0]
                
                action_count = len(analysis_actions)
                logger.info(f"Found {action_count} analysis actions for reaction {reaction_id}")
            except (IndexError, KeyError) as e:
                logger.warning(f"No analysis actions defined for reaction {reaction_id}, recipe {recipe_type}: {str(e)}")
            ...
            for index, analysis_action in enumerate(analysis_actions):
                ...
                logger.info(f"Processing analysis action {index+1}/{len(analysis_actions)}: {action_type} {action_number}")
                
                if action_type == "add":
                    self.process_add_action(analysis_action, action_number, index, analysis_actions,
                                          actionsession_obj, reaction_obj, reaction_id)
                else:
                    logger.warning(f"Unknown analysis action type: {action_type}")
                    
        except (KeyError, IndexError) as e:
            logger.error(f"Error accessing analysis recipe data for reaction {reaction_id}: {str(e)}")
            self.add_command(f"\n\t# Error finding analysis actions for reaction {reaction_id}: {str(e)}")
    
    def process_add_action(self, analysis_action, action_number, index, analysis_actions,
                         actionsession_obj, reaction_obj, reaction_id):
        """
        ...
        reaction_id : int
            The reaction ID
        """
        logger.info(f"Processing analysis add action {action_number} for reaction {reaction_id}")
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error accessing plate types in analysis action: {str(e)}")
                from_plate_type = add_action_obj.fromplatetype
                to_plate_type = add_action_obj.toplatetype
                logger.info(f"Using plate types from database: from={from_plate_type}, to={to_plate_type}")
                
            logger.info(f"Analysis transfer: from={from_plate_type}, to={to_plate_type}")
            ...
            logger.info(f"Adding analysis transfer command: {aspirate_plate_name}:{aspirate_well_index} → " +
            ...
            if add_action_obj.mix:
                ...
                logger.info(f"Adding mix command in destination well ({mix_repetitions} repetitions)")
                self.add_command(
            ...
            logger.info("Analysis transfer complete (well status not updated for analysis plates)")
            
        except AddAction.DoesNotExist:
            logger.error(f"Add action not found for reaction {reaction_id}, action number {action_number}")
